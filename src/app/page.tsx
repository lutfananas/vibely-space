...
/* ============================================================
   CARD DECOR — hiasan cute untuk card putih polos.
   Isi: 2 blob gradient lembut (pink+sky), sparkle ✦✧ animasi,
   dan cluster dot pattern. Super ringan (CSS only).
   Pakai di dalam card yang punya `relative overflow-hidden`.
   ============================================================ */

function CardDecor({ tone = 'pink' }: { tone?: 'pink' | 'blue' }) {
  const primary = tone === 'blue' ? '56, 189, 248' : '244, 114, 182'
  const secondary = tone === 'blue' ? '244, 114, 182' : '56, 189, 248'
  return (
    <>
      {/* blob gradient utama — pojok kanan atas */}
      <div
        className="absolute -top-12 -right-12 w-36 h-36 rounded-full pointer-events-none"
        style={{ background: `radial-gradient(circle, rgba(${primary},0.16) 0%, rgba(${primary},0) 70%)` }}
      />
      {/* blob gradient sekunder — pojok kiri bawah (warna pasangan) */}
      <div
        className="absolute -bottom-14 -left-12 w-40 h-40 rounded-full pointer-events-none"
        style={{ background: `radial-gradient(circle, rgba(${secondary},0.12) 0%, rgba(${secondary},0) 70%)` }}
      />
      {/* sparkle animasi */}
      <span className={`absolute top-3.5 right-4 text-[11px] animate-sparkle pointer-events-none ${tone === 'blue' ? 'text-sky-300' : 'text-pink-300'}`}>✦</span>
      <span
        className={`absolute top-8 right-9 text-[8px] animate-sparkle pointer-events-none ${tone === 'blue' ? 'text-pink-300' : 'text-sky-300'}`}
        style={{ animationDelay: '0.9s' }}
      >✧</span>
      <span
        className={`absolute bottom-4 left-5 text-[9px] animate-sparkle pointer-events-none opacity-70 ${tone === 'blue' ? 'text-sky-300' : 'text-pink-300'}`}
        style={{ animationDelay: '1.6s' }}
      >✦</span>
      {/* cluster dots kecil */}
      <div
        className="absolute bottom-3 right-4 w-12 h-7 dot-pattern opacity-50 pointer-events-none"
        style={{ backgroundSize: '8px 8px', maskImage: 'linear-gradient(135deg, black 30%, transparent 100%)', WebkitMaskImage: 'linear-gradient(135deg, black 30%, transparent 100%)' }}
      />
    </>
  )
}
      ...
      <div className="relative max-w-6xl mx-auto px-4">
        ...
        <div ref={ref} className="grid grid-cols-2 lg:grid-cols-4 gap-4 sm:gap-6">
          {FAST_STATS.map((s, i) => (
            ...
            >
              <CardDecor tone={s.tone === 'blue' ? 'blue' : 'pink'} />
        ...
        <div className="relative grid sm:grid-cols-3 gap-6">
          ...
          {VIBELY_LAYERS.map((layer, i) => (
            <Reveal key={i} delay={i * 120}>
              <div className="relative bg-white rounded-[2rem] border border-pink-100 shadow-cute p-7 text-center h-full hover:-translate-y-2 hover:shadow-cute-lg transition-all duration-300 overflow-hidden">
                <CardDecor tone={layer.tone === 'blue' ? 'blue' : 'pink'} />
        ...
        <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-5">
          {SAFETY_ITEMS.map((item, i) => (
            <Reveal key={i} delay={i * 90}>
              <div className={`relative bg-white rounded-[1.75rem] p-6 shadow-cute border h-full hover:-translate-y-1.5 hover:shadow-cute-lg transition-all duration-300 overflow-hidden ${
                item.tone === 'blue' ? 'border-sky-100 hover:border-sky-300' : 'border-pink-100 hover:border-primary/40'
              }`}>
                <CardDecor tone={item.tone === 'blue' ? 'blue' : 'pink'} />
                <span className={`relative w-11 h-11 rounded-xl flex items-center justify-center text-xl mb-4 ${
          ...
          <div className="max-w-6xl mx-auto px-4">
            ...
            <div className="grid lg:grid-cols-2 gap-10 lg:gap-14 items-center">
              ...
              <div className="space-y-4">
                <Reveal delay={100}>
                  <div className="relative bg-white rounded-3xl p-6 shadow-cute border border-pink-100 overflow-hidden">
                    <CardDecor tone="pink" />
                    <p className="relative text-base sm:text-lg text-foreground/90 leading-relaxed">
                ...
                <Reveal delay={200}>
                  <div className="grid sm:grid-cols-2 gap-4">
                    {[
                      ...
                      <div key={i} className={`relative bg-white rounded-3xl p-5 shadow-cute border hover:-translate-y-1 transition-all duration-300 overflow-hidden ${
                        item.tone === 'blue' ? 'border-sky-100 hover:border-sky-300' : 'border-pink-100 hover:border-primary/40'
                      }`}>
                        <CardDecor tone={item.tone === 'blue' ? 'blue' : 'pink'} />
                        <span className={`relative w-10 h-10 rounded-xl flex items-center justify-center text-xl mb-3 ${
                          item.tone === 'blue' ? 'bg-sky-50' : 'bg-pink-50'
                        }`}>{item.emoji}</span>
        ...
        <section className="py-16 sm:py-24 relative">
          <div className="max-w-5xl mx-auto px-4">
            ...
            <div className="grid sm:grid-cols-3 gap-6 relative">
              ...
              {STEPS.map((step, i) => (
                <Reveal key={i} delay={i * 120}>
                  <div className="relative bg-white rounded-[2rem] border border-pink-100 shadow-cute p-7 text-center hover:-translate-y-2 hover:shadow-cute-lg transition-all duration-300 overflow-hidden">
                    <CardDecor tone={i % 2 === 0 ? 'pink' : 'blue'} />
                    <div className="relative mx-auto mb-4 w-16 h-16">
            ...
            <div className="grid sm:grid-cols-2 gap-5">
              <Reveal delay={100}>
                <a href="https://instagram.com/vibely.space" target="_blank" rel="noopener noreferrer" className="group block">
                  <Card className="rounded-[2rem] border-2 border-pink-100 hover:border-pink-300 transition-all duration-300 hover:-translate-y-1.5 hover:shadow-cute-lg shadow-cute bg-white overflow-hidden relative">
                    <CardDecor tone="pink" />
                    <CardContent className="p-7 sm:p-8 flex items-center gap-5 relative">
              ...
              <Reveal delay={200}>
                ...
                >
                  <Card className="rounded-[2rem] border-2 border-sky-100 hover:border-sky-300 transition-all duration-300 hover:-translate-y-1.5 hover:shadow-cute-lg shadow-cute bg-white overflow-hidden relative">
                    <CardDecor tone="blue" />
                    <CardContent className="p-7 sm:p-8 flex items-center gap-5 relative">
